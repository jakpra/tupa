...
class Model:
    ...
    def init_labels(self, axis):
        labels = self.feature_params.get(axis)
        if labels is None:
            labels = self.label_param_def(axis).create_from_config()
            if self.is_neural_network:
                self.feature_params[axis] = labels
        self.init_param(axis)
        labels.init_data()
        return labels.data
