    ...
    def add_to_l1(self, l0, l1, parent, edge_categories, labeled, node_labels):
        ...
        if self.text:  # For Word terminals (Punctuation already created by add_punct for parent)
            if parent.node is not None:
                tag = self.incoming[0].tag
                if tag != EdgeTags.Terminal:
                    Config().print(tag, level=0)
                if self.node is None:
                    self.node = parent.node.add_multiple([(tag if tag.startswith(EdgeTags.Terminal) else EdgeTags.Terminal, "", "", "")], self.get_terminal(l0)).child
                elif self.node not in parent.node.children:
                    parent.node.add_multiple([(tag if tag.startswith(EdgeTags.Terminal) else EdgeTags.Terminal, "", "", "")], self.node)
