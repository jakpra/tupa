        ...
        if self.text:  # For Word terminals (Punctuation already created by add_punct for parent)
            if parent.node is not None:
                if self.node is None:
                    self.node = parent.node.add_multiple([(EdgeTags.Terminal, "", "", "")] + [(e.refinement, "", "", e.tag) for e in self.incoming], self.get_terminal(l0)).child
                elif self.node not in parent.node.children:
                    parent.node.add_multiple([(EdgeTags.Terminal, "", "", "")] + [(e.refinement, "", "", e.tag) for e in self.incoming], self.node)
